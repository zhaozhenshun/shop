...
function M_BeforeRemove(treeId, treeNode) {
	alertMsg.confirm("确定删除 "+ treeNode.name +" 吗?", {
		okCall: function(){
			ajaxPost("productCateDelete.do","id="+treeNode.id, "");
			 return true;
		}
	});
   return false;
...
function ZtreeClick(event, treeId, treeNode) {
	$(this).dialog({id:"productCateEdit"+treeNode.id, url:'productCateEdit?id='+treeNode.id, title:"修改",mask:true});
}
</script>
<div class="bjui-pageHeader">
        <ul class="bjui-searchBar">
            <li>
			<a href="productCateAdd" class="btn btn-default" data-toggle="dialog" data-width="800" data-height="400" data-id="dialog-mask" data-mask="true">添加</a>
			</li>
        </ul>
</div>
